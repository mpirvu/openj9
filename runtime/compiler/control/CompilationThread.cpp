...
TR::CompilationInfoPerThreadBase::shouldPerformLocalComp(const TR_MethodToBeCompiled *entry)
   ...
   // Expensive compilations are always sent remotely
   // AOT compilations are considered expensive
   ...
   if (optLevel == warm &&  bcsz > 6) // For bcsz==7 mem=7.5 MB which is deemed too much
      return false;
   ...
   if (freePhysicalMemorySizeB < 20 * 1024 * 1024) // 10MB <= freeMem < 20MB
      ...
      doLocal = (optLevel <= cold) && (bcsz <= 4); 
      }
   else if (freePhysicalMemorySizeB < 100 * 1024 * 1024) // 20MB <= freeMem < 100MB
      {
      // All methods at noOpt, very small methods at cold/warm and medium sized methods at cold (expected to take 2.5MB max)
      doLocal = (optLevel < cold) || (bcsz <= 4) || (optLevel == cold && bcsz <= 31);
      }
   else // freeMem >= 100MB
      {
      // All methods at noOpt/cold and small methods at warm (expected to take 6MB max)
      doLocal = true; // large methods at warm have already been filtered out above
      }
   // Now, look at CPU
   if (doLocal)
      {
      ...
      if (cpuEntitlement < 100.0)
         ...
         if (optLevel >= warm || bcsz >= 32)
            ...
            if (bcsz <= 7)  // Small methods at noOpt/cold should be local
            ...
            else // If we have CPU available we could keep local even medium sized cold compilations
               {
               ...
               if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                  TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "shouldPerformLocalComp %d: cpuEntitlement=%.1f idle=%d usage=%d", 
                     doLocal, cpuEntitlement, cpuUtil->getCpuIdle(), cpuUtil->getVmCpuUsage());
               }
            }
         }
      else if (cpuEntitlement < 350.0)
         {
         // Medium sized methods at noOpt/cold
         doLocal = (optLevel <= cold) && (bcsz <= 31); 
         }
      else
         {
         // All noOpt/cold methods and small warm methods
         doLocal = (optLevel <= cold) || (bcsz <= 5);
         }
      }
