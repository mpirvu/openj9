...
void MessageBuffer::expandIfNeeded(uint32_t requiredSize)
{
...
void MessageBuffer::expand(uint32_t requiredSize, uint32_t numBytesToCopy)
{
    TR_ASSERT_FATAL((requiredSize > _capacity), "requiredSize %u has to be greater than _capacity %u", requiredSize,
        _capacity);
    TR_ASSERT_FATAL((numBytesToCopy <= _capacity), "numBytesToCopy %u has to be less than _capacity %u", numBytesToCopy,
        _capacity);
...
uint32_t MessageBuffer::writeData(const void *dataStart, uint32_t dataSize, uint8_t paddingSize)
{
...
uint32_t MessageBuffer::computeRequiredCapacity(uint32_t requiredSize)
{
...
void MessageBuffer::tryFreePersistentAllocator()
{
    if (J9::PersistentInfo::_remoteCompilationMode != JITServer::CLIENT)
        return;
